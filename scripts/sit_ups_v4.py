...
class sitUps(object):
    ...
    def main(self, args):
        ...
        while True:
            ...
            #     frame_rate = 1 / (time.time() - start_inner_time)
            ...
            if has_display:
                ...
                if (angle_hks <= 70  or x_diff_elbow_knee < 0) and  not raise_feet  and start:
                    self.text = "count_{}".format(self.num_of_std)
                    self.count(self.frame, self.text , num_of_frame, root, video)
                    self.num_of_std += 1
                    start = False
    ...
    def frame_rate_func(self):

        print('frame_rate', self.cnt)
        self.cnt = 0
        t = Timer(1.1, self.frame_rate_func)
        t.start()
        k = cv2.waitKey(1)
        if k == 27:
            os._exit()
    ...
    def cal_angle(self, pts, flag):
        try:
            ...
            if flag == 'start':
                ...
                if x_distance_elblow_knee <= 0.35 * x_distance_ankle_knee:
